...
const isValidPwd = (val) => {
    return val.match(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!#$%^&-+=()]).{8,16}$/);
}
...
const validateForm = () => {
  ...
  // check password for error ans success
  if(pwdValue === ''){
    setMessage(messageType.ERROR, pwd, 'Password is Required');
    isFormValid = false;
  }else if(!isValidPwd(pwdValue)){
    setMessage(messageType.ERROR, pwd, 'Passwrod should contain uppercase, lowercase, special and numeric characters');
    isFormValid = false;
  }else{
    setMessage(messageType.SUCCESS, pwd)
  }
